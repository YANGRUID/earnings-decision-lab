...
def get_options_provider(settings: Settings, override: str | None = None) -> OptionsDataProvider:
...
def _build_options_provider(name: str, settings: Settings) -> OptionsDataProvider | None:
...
def build_options_provider_chain(
    ...
    for name in order:
        provider = _build_options_provider(name, settings)
...
def _build_market_data_provider(name: str, settings: Settings) -> MarketDataProvider | None:
    ...
    if name == "tiingo" and settings.tiingo_api_key:
        return TiingoMarketDataProvider(api_key=settings.tiingo_api_key)
    if name == "alpha_vantage" and settings.alpha_vantage_api_key:
        return AlphaVantageMarketDataProvider(api_key=settings.alpha_vantage_api_key)
    return None
...
def build_market_data_chain(
    ...
    for name in order:
        provider = _build_market_data_provider(name, settings)
        if provider is not None:
            providers.append((name, provider))
