...
[evaluation.md](evaluation.md) — not duplicated here. Summary of what that document itself
documents as a limitation of the evaluation:

- **fact_coverage is a substring match, not a semantic judge** — a correct answer that
  reformats a number registers as a miss. A real, structural false-negative risk.
- **No LLM-as-judge secondary layer**, even though the original project scope allowed one —
  the fact_coverage caveat above already covers the gap it would have caught, and adding a
  second model to grade the first adds real cost for a benefit that's already documented
  honestly rather than papered over.
- **Per-item cost/token accounting only exists for the agent-orchestration category** —
  `answer_question` and `extract_guidance` don't currently plumb `TokenUsage` back to their
  callers the way `AgentOrchestrator.ExecutionTrace` does, so the RAG-answer and
  structured-extraction categories' real dollar cost isn't reported per item.
- **Retrieval is the measured bottleneck for the AI Research feature as a whole**
  (Recall@5 ≈ 0.35), with a specific, verified cause: boilerplate section-opener text and
  repeated numeric tables score competitively against the actual narrative sentence that
  answers a question, on both the vector and keyword side of RRF. This is the same
  no-relevance-floor/no-reranker gap already documented under Phase 5 below — this
  evaluation is what turned that from a guess into a measured fact, not a new gap.
- **`evaluation/results/*.json` (the raw per-run output) is gitignored, not committed** — the
  numbers in evaluation.md are copied from a real run, not regenerated by CI on every commit,
  so they can drift from HEAD as the codebase changes after a given run. Re-running
  `evaluation/scripts/run_all.py` is the source of truth for "right now," not this file.
