...
A real, incremental commit history is more useful later than a single squashed drop — each
commit is a checkpoint that can be bisected, reverted, or reread to understand why a decision
was made at the time, not just what the code looks like now.

**Why MIT license?** Simplest permissive choice for a personal project; no reason to restrict
reuse of the code (as opposed to any data, which has its own licensing constraints — see
...
project than this system's current scale justifies; regex heading detection on cleaned text is
...
read than an extra build-pipeline dependency; the drift risk is real and stated plainly rather
than hidden (see docs/limitations.md), not solved prematurely.
